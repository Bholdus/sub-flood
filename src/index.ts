...
function seedFromNum(seed: number, salt: number): string {
  return "//user//" + (`${salt}` + "0000" + `${seed}`);
}
...
async function run() {
  ...
  let FINALISATION_ATTEMPTS = argv.finalization_attempts
    ...
    : 5;
  let SALT = argv.salt ? argv.salt : 1;
  let ACCOUNT = argv.account ? argv.account : "//Alice";
  ...
  for (let i = 0; i <= TOTAL_USERS; i++) {
    let stringSeed = seedFromNum(i, SALT);
  ...
  console.log(`Endowing all users from ${ACCOUNT} account...`);
  let aliceKeyPair = keyring.addFromUri(ACCOUNT);
  let aliceNonce = (
    await api.query.system.account(aliceKeyPair.address)
  ).nonce.toNumber();
  let keyPairs = new Map<number, KeyringPair>();
  console.log(`${ACCOUNT} nonce is ` + aliceNonce);

  let finalized_transactions = 0;

  for (let seed = 0; seed <= TOTAL_USERS; seed++) {
    let keypair = keyring.addFromUri(seedFromNum(seed, SALT));
    keyPairs.set(seed, keypair);

    // should be greater than existential deposit.
    let transfer = api.tx.balances.transfer(
      keypair.address,
      api.consts.balances.existentialDeposit.mul(new BN(100))
    );

    let receiverSeed = seedFromNum(seed, SALT);
    console.log(`${ACCOUNT} -> ${receiverSeed} (${keypair.address})`);
  ...
  console.log(`All users endowed from ${ACCOUNT} account!`);
